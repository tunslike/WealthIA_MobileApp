...
import { HeaderBar, ChatCard, Loader } from '../components';

const ProviderChat = ({route, navigation}) => {


    const { providerID, clientName, channelName, message, RequestID, msgReadStatus } = route.params
    ...
    postMessageReply = async () => {
        ...
        setLoading(true);
         ...
         .post(appConfig.BASE_CONFIG.base_url + 'messages/postMessageResponse', {
             ...
             msgRuestID: RequestID,
             message: messageReply,
             response_by: providerID,
    ...
    loadMessageResponses = () => {
        ...
        // Set Axios 
        axios
        .get(appConfig.BASE_CONFIG.base_url + 'messages/loadMessageResponses/' + RequestID, {
            ...
            >
                  ...
                  >
                    ...
                    {allMessages.map((prop, key) => {
                        return (
                            <View style={{
                                backgroundColor: (providerID == prop.RESPONSE_BY) ? '#ebf4ff' : '#e8e6dc',
                                marginBottom: 15,
                                padding:10,
                                alignItems: (providerID == prop.RESPONSE_BY) ? 'flex-start' : 'flex-end',
                                width: '85%',
                                borderBottomLeftRadius: (providerID == prop.RESPONSE_BY) ? 0 : 7,
                                borderTopLeftRadius: (providerID == prop.RESPONSE_BY) ? 0 : 7,
                                borderTopRightRadius: (providerID == prop.RESPONSE_BY) ? 7 : 0,
                                borderBottomRightRadius: (providerID == prop.RESPONSE_BY) ? 7 : 0,
                                justifyContent: 'flex-start',
                                alignSelf: (providerID == prop.RESPONSE_BY) ? 'flex-start' : 'flex-end'
